...
export default class Search{
    //1.store the dom elements and keep track of them.
    constructor(){
       ...
       if(posts.length){
              ...
              ${posts.map(post=>{
                  ...
                  <img class="avatar-tiny" src="${post.author.avatar}"> <strong>${post.title}</strong>
                    <span class="text-muted small">${post.author.username} on ${postDate.getDate()}/${postDate.getMonth()}/${postDate.getFullYear()}</span>
                </a>`
              }).join('')}
         </div> `)
       }  else{
